...
export function DialogModel(props: { providerID?: string }) {
  ...
  const kiloProvider = sync.data.provider.find((provider) => provider.id === "kilo")
  const kiloModels = kiloProvider?.models ?? {}
  const kiloRank = createMemo(() => new Map(
    Object.entries(kiloModels).map(
      ...
      flatMap((provider) =>
        pipe(
          ...
          sortBy(
            ...
            (x) => (x.value.providerID === "kilo" ? kiloRank().get(x.value.modelID)! : 0),
            // kilocode_change end
            (x) => x.footer !== "Free",
            (x) => x.title,
          ),
